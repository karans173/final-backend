...
def upload_csv_to_supabase(file_path, file_name_in_bucket):
    """
    Upload local CSV file to Supabase Storage bucket.
    """
    ...
    if response.status_code in [200, 201]:
        ...
        # Public URL depends on bucket policy: if bucket is public, use:
...
def run_stock_prediction(ticker, feature='Close', time_steps=60, test_size=0.2, epochs=50, forecast_days=30):
    ...
    future_df.to_csv(csv_filename)

    uploaded_url = upload_csv_to_supabase(csv_filename, csv_filename)
    if uploaded_url:
